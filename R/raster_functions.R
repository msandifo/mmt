...
#' @examples
bytscl <-function(ras) {
  round(255.*(ras - minValue(ras))/(maxValue(ras)-minValue(ras)),0)}
...
set_axes_trad<-function( e.ras,
  ...
  e.diff  <- max( diff(e.ras[1:2]) , diff(e.ras[3:4] ))

  if (double){
    gl.mar = sp::gridlines(e.sp,
                           #easts = c(e.ras[1:2],  e.ras[1:2] + diff(e.ras[3:4])*c(axis.width,-axis.width)),
                           # easts = c(e.ras[1:2],  e.ras[1:2] + diff(e.ras[1:2])*c(axis.width,-axis.width)),
                           # norths =c(e.ras[3:4],  e.ras[3:4] + diff(e.ras[3:4])*c(axis.width,-axis.width)),
                           easts = c(e.ras[1:2],  e.ras[1:2] + e.diff*c(axis.width,-axis.width)),
                           norths =c(e.ras[3:4],  e.ras[3:4] + e.diff*c(axis.width,-axis.width)),
                           ndiscr = ndiscr)
  } else{
    gl.mar = sp::gridlines(e.sp,
                           #easts = c( e.ras[1:2] + diff(e.ras[3:4])*c(axis.width,-axis.width)),
                           # easts = c( e.ras[1:2] + diff(e.ras[1:2])*c(axis.width,-axis.width)),
                           # norths =c( e.ras[3:4] + diff(e.ras[3:4])*c(axis.width,-axis.width) ),
                           easts = c( e.ras[1:2] + e.diff*c(axis.width,-axis.width)),
                           norths =c( e.ras[3:4] + e.diff*c(axis.width,-axis.width) ),
                           ndiscr = ndiscr)
  }
  return(gl.mar )
}
                  ...
                  ...)
{
 ...
 #
   if( add==TRUE) {
    par(mar=mar, mgp=mgp,new=TRUE, xpd=xpd)
  } else
  {
    par( mar=mar, mgp=mgp, xpd=xpd)
...
build_raster_stack <- function(ras,  #raster obj.
  ...
  if (length(shade)==1)  shade= c(shade, 10,320)
  if (length(shade)==2)  shade= c(shade,  320)
...
trad_axes_poly <- function(e.ras, #etxent  or raster class
  ...
  #print(e.ras)
  ax.width.plot <- max( diff(e.ras[1:2])*axis.width, diff(e.ras[3:4])*axis.width)
  # ax.width.plot.x <-    diff(e.ras[1:2])*axis.width
  # ax.width.plot.y <-    diff(e.ras[3:4])*axis.width
  ax.width.plot.x <-    ax.width.plot
  ax.width.plot.y <-    ax.width.plot
  ...
  if (!test){
    ...
    ysb <-rep(c(rep(e.ras[3],  times=nseq),  rep(e.ras[3]+  ax.width.plot.y,  times=nseq),e.ras[3]), npoly.c)

    y.poly.bottom <-  matrix(ysb, nrow = npoly.r, ncol = npoly.c)  %>% signif(  digits = 5)

    yst <-rep(c(rep(e.ras[4],  times=nseq),  rep(e.ras[4]-  ax.width.plot.y,  times=nseq),e.ras[4]), npoly.c)
    ...
    xsl <-rep(c(rep(e.ras[1],  times=nseq),  rep(e.ras[1] +  ax.width.plot.x,  times=nseq),e.ras[1]), npoly.c)
    x.poly.left <-  matrix(xsl, nrow = npoly.r, ncol = npoly.c)  %>% signif( digits = 5)
    ysr <-rep(c(rep(e.ras[2],  times=nseq),  rep(e.ras[2] - ax.width.plot.x,  times=nseq),e.ras[2]), npoly.c)
...
plot_axes_trad <- function(e.ras, proj=NA, ax.int=5,  add = FALSE, lwd=1, col="black", fg="black", cex=3, grid=F,width=2,
                           ...
                           extend=0.02, axis.width=0.007, stringc=TRUE, degrees=TRUE, double=TRUE, poly=TRUE, test=FALSE, label.int=1,
                           background.col="white" # for polyon extend colors
){
  ...
  sp::plot(ax.poly.back, add = add, lwd=0.0001, col=background.col, fg=background.col, border=background.col)
  ...
  #CRS( proj)@projargs
  # print(g.labs[seq(1,label.int, length(g.labs@data$labels)), ])
  #graphics::text(labels(gl, CRS(legend.crs), side = 2:3 ), cex=cex )
  #graphics::text(g.labs[seq(1, length(g.labs@data$labels),label.int), ], cex=cex )
...
#'
#' @examples
convertPts <- function(x,
                       y=y,
                       from=CRS("+init=epsg:4326"),
                       to=CRS("+proj=laea +lat_0=-45.7 +lon_0=132.9 +ellps=WGS84") ,
                       xy=T,
                       single=F){
  if ( is.list(x) | is.data.frame(x) ) {
    d= as.data.frame(x)
    names(d)<- c("lon", "lat")}
  else if  (class(x)=="numeric" & length(x)==2) {d = data.frame(lon=x[1], lat=x[2])}
...
extractLines<-function( ras, x=c(150,140), y=c(-11.5,-43.5), n=100, plot=TRUE, df=TRUE , km=TRUE, gc=T){
 ...
 # print(str(line.df))
  my.lines<-raster::extract(ras, line.df, df=TRUE)
#  print(head(my.lines))
  line.df <- bind_cols(line.df, raster::extract(ras, line.df, df=TRUE))
...
BlRe.colours <- function(blues=6, reds=6, reverse=F, drop=NA){
 ...
 else cols =c(head(colfunc(blues), -1),colfunc1(reds) )
